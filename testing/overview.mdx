...
| Integration | Playwright (no browser) | `npm run test:integration` | Schema validation, module imports |
...
<Steps>
  ...
  <Step title="Integration tests">
    `npm run test:integration` runs on every push. Validates schema structure, Storybook config, and module conventions without spinning up a browser.
  </Step>
